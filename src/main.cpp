...
void drawPanel(const Panel& panel, size_t index, float cx, float cy) {
    constexpr float unit = 1.f / 900.f;
    const auto& p = panel.layout;
    glPushMatrix();
    glTranslatef((p.x + p.width/2 - cx)*unit, -(p.y + p.height/2 - cy)*unit, 0);
    ...
    rectangle(-w/2-.02f,-h/2-.02f,w+.04f,h+.04f,0);
    glColor3f(.065f,.085f,.12f);
    rectangle(-w/2,-h/2,w,h,.005f);
    if (panel.width && !panel.failed) {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0,1); glVertex3f(-tw/2,-th/2,.01f);
        glTexCoord2f(1,1); glVertex3f(tw/2,-th/2,.01f);
        glTexCoord2f(1,0); glVertex3f(tw/2,th/2,.01f);
        glTexCoord2f(0,0); glVertex3f(-tw/2,th/2,.01f);
        glEnd(); glDisable(GL_TEXTURE_2D);
    } else {
        for (int row=0; row<8; ++row) {
            glColor3f(.2f,.25f,.32f);
            rectangle(-w*.44f,h*.32f-row*h*.09f,w*.7f,h*.025f,.01f);
        }
    }
    glPopMatrix();
}
int preview(std::vector<Panel>& panels, bool smoke) {
    ...
    while (running) {
        ...
        glRotatef(pitch,1,0,0); glRotatef(yaw,0,1,0); glTranslatef(panX,panY,-distance);
        for (size_t i=0;i<panels.size();++i) drawPanel(panels[i],i,cx,cy);
...
int main(int argc,char** argv) {
    try {
        bool smoke=false,list=false; int fps=30;
        ...
        for (int i=1;i<argc;++i) {
            ...
            if (arg=="--help") { std::cout << "Usage: omarchy-xr [--capture OUTPUT ... | --layout FILE | --list-outputs] [--fps 1..60] [--smoke-test]\nRight-drag: look; middle-drag: pan; wheel: zoom; F: fit; R: recenter; Esc: exit\n"; return 0; }
