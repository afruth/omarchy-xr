...
struct Card {
    ...
    std::string identity() const {return generation+"/"+key;}
};
...
inline Json parse(const std::string& data) {
    if(data.empty() || data.size()>256*1024) return {nullptr,json_object_put};
...
inline std::optional<Card> readCard(const std::string& data,double wallMs) {
...
inline std::string readFile(const std::string& path) {
    std::ifstream file(path,std::ios::binary);std::string data(256*1024+1,'\0');
