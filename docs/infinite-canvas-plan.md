...
- Each session posts `screencast`/`screencastv2` IPC events with a 500 ms stop timer **[verified: `ScreenshareSession.cpp:137-152`]**: idle sessions are kept alive without `copy` and rate changes are hysteresis-limited (§4.4) so the omarchy-shell recording indicator does not flicker.
...
| S11 | Output scale legibility. | All of 1.0 / 1.25 / 1.5 comfortable; **1.0 is the default** (D3). Recheck in M2 once the viewer resamples onto the cylinder. |
...
| `src/canvas_placement.hpp` | `category()`, `placeNew()` (periodic ring search, parent-aware), `arrange()`, `neighbour(dir)`, `nudge()`, `snap()`, `rowFor(y)`. Pure. |
...
- `<state>/canvas.json` (Studio) → `<state>/canvas.tsv` (renderer, hot-reloaded every 250 ms): `# canvas v1 fps radius gapPx dimUnmatched labelDeg outputScale captureBudgetMpix adoptPolicy` plus `exclude <pid|class>` rows. `outputScale` is the canvas output's scale (1.0/1.25), which the renderer needs for the Fill buffer→logical conversion; the output refresh (60/120) is backend-only, applied through `hl.monitor`, and is not carried in `canvas.tsv`.
- `<state>/canvas-session.json` (Studio): per-window origin state for restore.
- `<state>/presentation.json`: `"renderMode": "monitors"|"canvas"`.
- Runtime mailboxes: `.windows`, `.cursor` (Lua→renderer), `.hover` v4 and `.tiers` (renderer→Lua: `v1 pid seq <address> sliver|park …`), `.mode` (backend→Lua: `v1 canvas|monitors <takeover 0/1> stamp`; the flag is the Studio switch for the optional chords, §6.5), `.search` (prompt→renderer: `v1 owner seq <hex text> <open 0/1> stamp`).
...
### M2 — Window capture hub + canvas scene, renderer-only (2–3 weeks)
...
### M3 — Control plane: Lua, backend, Studio toggle, input (2–3 weeks)
...
**Tests**: `capture_governor.cpp` (ladder arithmetic against the S1c table, budget never exceeded, caps, hysteresis, self-calibration up/down, sliver threshold → 2 in flight, indicator rate), `capture_plan.cpp` regression, live cadence.
**Shippable**: yes.

### M6 — Live mode switch, preservation hardening, docs, release (1–2 weeks)
**Changes**: `mode:` datagram + single-process scene swap; backend live switch with `relocate_workspaces(exclude)`; notification inside-ring routing + tests; HUD in non-stereo (D6); `ensureLease`/`finish` audit; stats consumers; docs, README, `package-release.py`; version bump.
**Acceptance**: switching modes while in stereo keeps SBS and the lease (`test_live_mode_switch_never_restarts_viewer_or_sdk`); full matrix green: `make check`, `check-san`, `check-notifications`, `check-workspace-focus`, `check-environment`, `check-preview`, `smoke`, `smoke-canvas`, `check-ui`, `check-lint`; manual checklist in both modes × {stereo, glasses-flat, windowed, spectator}.
...
- Request→ready p50: ≤ 2 canvas output frames (33 ms at 60 Hz, 17 ms at 120 Hz; measured 25 / 15 ms); the governor shrinks the budget when this is exceeded.
...
12. Screencast event flood — **accepted**: sessions kept alive, tier-change rate limit, indicator check deferred to M5 acceptance ("recording indicator steady").
...
10. Dropped phantomat features — **accepted**: nudge, summon, mouse drag in Overview, pin, radar strip, hold-to-show Alt-Tab, F1 help; head-drag **deferred** (no hold-able gesture available).
