...
- Selectable render mode `monitors` (today) | `canvas` (new), persisted by Studio; switchable when stopped since M3, and live (one renderer process, `mode:` datagram) since M6 — shipped.
...
| rows | ~~row 0 at eye height, at most ±1 extra row~~ **M8: height unbounded**; `rowHeight` is only the arrange grid; the view scrolls vertically (`Camera::scrollY`) | – |
...
```
x' = focusX + wrap(x − focusX)·zoom     y' = focusY + (y − focusY)·zoom − scrollY
w' = w·zoom                              h' = h·zoom
```

**Vertical scroll (M8).** `scrollY, targetScrollY` (projected px, eased with the focus) translate the whole canvas on the cylinder surface: `y' = 0` is eye level, so windows stay upright and undistorted at any scroll and text at eye level stays 1:1. `Scene::focusOn(x, y)` sets `targetFocusX/Y = (x, y)` and `targetScrollY = y`, which puts the canvas point at eye level; every landing uses it. `scrollBy(dy)` moves `targetScrollY` by projected px (constant screen speed at any zoom) and clamps it so the eye level stays within the live windows' vertical extent plus half a view on either side (`scrollLimits()`; with no windows `±viewH/2`). The zoom-anchor math (`zoomAt`, `holdAnchor`) is untouched: `scrollY` is a constant offset under it. Unprojection (`headingPoint`, `latch`, `lookPoint`, the radar) adds `scrollY` back.
...
**Persistence.**
- `<state>/canvas-memory.tsv` (renderer, phantomat format + `camera\tx\ty\tzoom[\tscroll]`; M8 writes the fifth field, a four-field row loads with `scroll = y`; window rows keep `y` unclamped, so pre-M8 files load unchanged).
...
- ~~Rows of `rowHeight` (850 px) around the ring … never more: ±25° pitch~~ **M8: the canvas is the full cylinder** — periodic in x, unbounded in y. `rowHeight` (850 px) is the arrange grid only; `gapPx` 60. Nothing clamps y (placement, drag, nudge, summon, memory).
- A window wider than `P/2` is clamped.
- **New window**: (1) session position, (2) `Memory::claim()` by class/title, (3) parent rule (dialogs: centred over the parent, staged), (4) `placeNew()`: start at the **look point** (M8: the canvas point under the view-centre ray, heading and pitch, `Scene::lookPoint()`; the camera focus in tests without a view), then a 2D spiral over the cells `(i, j)` of step `(w + gap, h + gap)` for rings `max(|i|,|j|) = 1..16`, ordered by distance from the start, ties by smaller `|j|`, then right, then down (deterministic, no `inBand`), periodic collision test, snap 20 px. Then a 300 ms halo pulse and, if outside the FOV, an edge cue (reuse `notification_draw.hpp` cue; both shipped in M6). The camera flies to it only if Hyprland focused it (§5.7).
- **Arrange** (`CTRL+A` in the prompt or the SDL window, Studio button, pose verb; from Work or Fill it ends in Overview): category grouping (categories by their most recent window, classes inside) + shelf-pack into a **2D block around the look point** (M8): rows of `rowHeight` on the grid row nearest the look point, then +1, −1, +2, −2 …, each row an arc `arcW = clamp(√(Σ(w+gap)(h+gap) · viewW/viewH), max(viewW, widest), P)` centred on the heading, packed left to right; what no row takes falls back to `placeNew` from the look point; undo/redo.
- **Move by hand** (M8: free in x and y, snapped, undoable, no row band): `SUPER+SHIFT+arrows` nudge one grid step; `Shift+Enter` in search = summon to the heading point; in Overview a mouse drag (2D or real mouse) moves the gazed window over the cylinder (shipped in M6 for the SDL window, i.e. the windowed preview and the `--display` host; the glasses keep nudge/summon/arrange, see M6 notes). Head-drag is deferred (no hold-able gesture in the current control set).
...
| **Overview** | `zoom = fitBounds(...)` (both axes: 1.0× view width, 0.9× view height, floor 0.08), focus = used-bounds centre (or of matches during search) scrolled to eye level, `panZ = 0`; labels on; non-gazed dimmed; radar strip shown (M8: with the vertical extent and the view rectangle) | gaze halos; flick-in / `fit_target` / click / Enter lands; wheel/pinch zooms about the latched anchor; 4-finger pan moves focus; **typing starts search** in an Overview entered from Work or Fill (the prompt opens with it; otherwise `SUPER+CTRL+G`, see M4 notes) |
...
Transitions are eased (τ 0.15 s zoom, 0.1 s rotation) and never jump; dwell is suppressed while `cameraMoving()` (the scroll included). Flick out: Fill→Work→Overview. Flick in: Overview→Work (land), Work→Fill. **M8:** every landing (`land`, confirm, search, switcher, neighbour, follow, chase, summon, Fill) scrolls the target to eye level through `focusOn`; the head looks up and down within the view as before, the scroll is what moves the cylinder beyond it.
...
- 4-finger pan (M8) → x: `focusX += dx·speed` with the aim (the head turns along the ring, as before); y: `scrollY −= dy·viewH/400` (grab semantics: content follows the finger, the aim stays). The gesture, its `.pan` codec and the flick/tap binds are unchanged.
- **Vertical scroll** (M8): `SUPER+wheel` (takeover of Omarchy's *Scroll active workspace*, behind the takeover switch, restored on exit) one notch = 0.2 view heights; `SUPER+CTRL+Page_Up/Page_Down` (free chords, always bound in canvas mode) = 0.8 view heights; windowed `PageUp/PageDown` the same; the middle-drag mouse pan scrolls vertically like the 4-finger pan. Mode **19 `scroll`** with the token `up|down|pageup|pagedown`. Recenter as in the table.
...
| 18 | `confirm` (M7) | the `fit_target` hotkey, the three-finger tap | – |
| 19 | `scroll` (M8) | `SUPER+mouse_up/mouse_down` (takeover, optional; Omarchy's *Scroll active workspace*), `SUPER+CTRL+Page_Up/Page_Down` (free **[verified M8: `hyprctl binds`]**, always bound) | hex `up|down|pageup|pagedown` |

`live_controls.hpp`: modes ≥ 8 accepted only in canvas mode (≤ 19 from M8), require the stamp, and require a target only for 14/15/19 (the `mode>7` check at `:194` and the "≥ 6 needs target" rule at `:202` change accordingly). `input_settings.py` adds `slash`/`period` to `KEYS` and exempts takeover chords from the clash validator in canvas mode.
...
### 6.1 Notifications
- `Hud` construction (`main.cpp:1068`) becomes `!posePath.empty()` so `--display` and windowed runs show cards (the uncommitted `drawEye` change already draws them in mono). **[decision D6 — resolved in M6: HUD whenever a pose socket exists]**
...
### M6 — Live mode switch, preservation hardening, docs, release — **DONE** (2026-09-26; glasses checklist pending)
...
**Tests**: `mode_switch.cpp` (new `build/test-mode-switch`, run offscreen by `check-workspace-focus` and `check-canvas`: real `mode:` datagrams for monitors → canvas → monitors → canvas, `mode:bogus` ignored, scene swap, environment kept, eye inside the ring, the `.mode` heartbeat, the forced `.stats` mode, three windows through the mailbox, `canvas-memory.tsv` saved on exit, an empty monitor scene that still ticks, the mailbox restarting at seq 1 on re-entry; the four refusals; a mono frame with the HUD differs from one without), `notification_space.cpp` (`insideRing` without occluders and with a tessellated 30-window 360° wall: lower band, inside the view, radius ≤ 2.1 on every frame of a 75° sweep, never `behind`, speed limits), `canvas_model.cpp` (`offView` and `dragScale`), `canvas_focus.cpp` (`newWindowCues`, `overviewDrag`, `notificationInsideRing`), `notification_controls.cpp` (`canvasFlicks`), `test_canvas.py` (`test_mode_persists_and_switches_live_while_viewing`, `test_live_mode_switch_never_restarts_viewer_or_sdk`, `test_live_outputs_never_overlap_during_the_switch`, `test_live_switch_falls_back_to_stop_first_without_an_ack`, `test_live_switch_refused_while_laptop_display_is_off`, `test_monitor_outputs_clear_a_live_canvas_output`).
**Delivered** in five work packages: WP1 the renderer's single-process scene switch (`PoseSocket::modeRequested`, `View::switchScene` with `enterCanvasScene`/`enterMonitorScene`, `viewerPath`/`canvasPath`, the forced `.stats` report, `LiveControls::setCanvasMode` clearing the canvas mailbox state) and the HUD in every run with a pose socket (D6), with `tests/mode_switch.cpp`; WP2 inside-ring notification routing (`space::Scene::maxRadius`/`lowerBand`, `Hud::draw` depth-off in canvas mode) and the deferred UX (new-window pulse, off-FOV edge cue, Overview mouse drag) with their unit tests; WP3 the backend/Studio live switch (`Manager.switch_live`, `request_mode`, `release_other_mode`, `restart_viewer`, `desktop_origin` counting the canvas output, the unlocked mode selector) with the `test_canvas.py` switch matrix; WP4 the docs ([`window-canvas.md`](window-canvas.md) with the consolidated end-to-end manual checklist, `architecture.md`, `README.md`, this entry and §8.2); WP5 the 0.4.0 release prep and the final gate run.
**Measured** (dev machine, TGL GT1; WP4 run, repeated by the WP5 release run with the same numbers): `make smoke-canvas` exits 0 on a temporary `SPIKE-canvas` output — 5 windows, the staged one 10 smoke frames (9 of them from the region source), the four parked idle, stereo validation distinct left/right images, the output and clients removed afterwards. `make check` 172 Python tests OK (3 skipped) plus every C++ unit suite; the WP1–WP3 gate runs (`check-san`, `check-notifications`, `check-workspace-focus`, `check-environment`, `smoke`, `check-canvas`, `check-canvas-preview`, `check-ui` 72 passed, ruff, mypy) were green; `check-preview` 7 of 7 stills pixel-identical; the lint baselines unchanged (luacheck 7 pre-existing warnings, `function_length.py` 2 pre-existing violations, qmllint only the pre-existing `Ui.BorderSurface` error). Live renderer sanity on a temporary `OMXRTEST-m6` headless output: `mode:canvas` logged `Scene: switched to canvas` and `.stats` reported `canvas` within 1 s; `mode:monitors` re-captured the output and reported `monitors`. WP5 (0.4.0, `./build/omarchy-xr --version` → `omarchy-xr 0.4.0`): the full gate list, `check-canvas-preview` and `check-canvas-live` (ladder-2, ladder-4, ladder-6 all-in-view and ladder-6 default view passed) green; no `SPIKE-` output or test client left behind.

#### M6 notes

Decisions taken while implementing M6:
- **Switch protocol**: the backend does the Hyprland side first (canvas output + migration, or `apply(layout)` + `viewer.tsv`), saves the mode, then sends `mode:<canvas|monitors>` to `pose.sock` and waits ≤ 3 s for `pose.sock.stats` to report the new `mode` with a fresh `time` (`time ≥ sent − 0.5 s`); only then are the other mode's outputs released. The acknowledgement is the existing `.stats` `mode` field with a forced report (`reportDue`, consumed by the next `recordWork`) instead of the 5 s cadence; no new mailbox or ack file. Forcing via `reportTime = −1e9` was rejected because it made the reported fps meaningless.
- **One switch point**: `tick()` applies the request right after `tracking.update()`. The new scene is built before the old one is torn down (the hub connects before the panels go; a canvas that cannot capture logs `Window canvas unavailable` and keeps the monitors). A monitor layout that cannot be captured yet leaves an empty monitor scene that keeps ticking and retries once a second (`Live layout update deferred`) until `viewer.tsv` changes or the outputs appear. `resetNavigation()` also zeroes pan and rotation, so the landing and fit start from a clean camera; leaving the canvas closes an open search prompt.
- **Refusals** (renderer): smoke runs, `--canvas-windows-file` runs, no canvas path, and canvas with controls < v6 (checked before any connect). The backend refuses a live switch while the laptop display is off, because the canvas restore needs a computer display.
- **Fallback (D10)**: without an acknowledgement the backend logs `live mode switch not acknowledged; restarting the XR view`, stops XR and restarts it in the same presentation (direct stereo via `start_dedicated`, else `start(present=…)`). The mode is persisted first, so a failed restart leaves the new mode selected with XR stopped. A stale `controls.version` cannot exercise this path, since Studio reads the same file and refuses first; the checklist stops the renderer with `SIGSTOP` instead.
- **Outputs never overlap**: `desktop_origin` counts a live canvas output as an obstacle (monitor outputs applied during canvas → monitors land to its right), and `CanvasSession.create` passes `clear_owned=True` so a canvas created during monitors → canvas lands right of the still-live monitor outputs. `place_monitors` no longer removes the canvas output (it would have dropped a live canvas before its windows were restored); the canvas leaves only through `CanvasSession.remove`. `relocate_workspaces` already excluded `-canvas` outputs.
- **Paths**: `viewerPath` (`viewer.tsv`) and `canvasPath` (`canvas.tsv`) are both derived from the state directory and replace the dual use of `layoutPath`; environment, tracking and gaze stay next to the start file.
- **`LiveControls::setCanvasMode`** always resets the heartbeat (the flip is announced at once) and clears the canvas mailbox state only on an actual change (the `.tiers` heartbeat test calls it while already in canvas mode).
- **Canvas cards use no occluders**: `space::clear()` tests the eye → card pyramid against the facets, which can never pass inside a 360° wall (before M6 canvas cards ended up overhead and invisible). Canvas mode sets `maxRadius = R − 0.3` and `lowerBand`, keeps `depth ≤ R − 0.3`, falls back to the cheapest in-band point instead of the overhead fallback, never marks a card `behind`, and draws the HUD depth-test-off after the scene. Monitor mode keeps the old paths behind the default flags (`check-preview` pixel-identical); `canvas::Scene::occluders()` stays for `scene_seam`.
- **New-window cues**: only windows added after the first adopted list (`Scene::primed`) pulse (300 ms via `haloTarget`), so neither a canvas start nor a live switch floods the ring; the edge cue reuses `notifications::drawCue` in the theme accent, lasts ≤ 3 s and expires as soon as the window centre is in view; "outside the view" is the pure `canvas::offView` of the projected position against the last cull heading.
- **Overview drag is SDL-only** (windowed preview and the `--display` host): the spectator receives no input and in the glasses the real pointer lives in the staged window. A press in Overview or Search starts it (pinned windows excepted, Work never drags); under 4 px of motion it stays a click at the press point. Drops snap to 20 px, may overlap (like nudge), revert outside the row band, are saved to `canvas-memory.tsv` and checkpointed for undo.
- **Lease loss** (`ensureLease`/`finish` audit): the canvas hub, labels and overlays (`releaseGpu`/`regenerate`), the cursor texture, environment, HUD and halo are released with the GL context in either mode; a switch never touches `DirectOutput`, the SDK or `Dedicated`.
- **Lua unchanged**: `xr-controls.lua` enters and leaves canvas mode from the renderer's `.mode` heartbeat, and `tests/controls.lua testCanvasActivation` already covers the flip within one active session.
- **Scene switch re-berths the HUD**: `resetNavigation` calls `Hud::resetPlacement`, so a card showing at monitor depth is placed afresh under the canvas limits (and vice versa) instead of snapping from a route planned for the old scene.
- **Packaging**: `package-release.py` ships `docs/window-canvas.md` to `/usr/share/doc/omarchy-xr` beside the existing guides; `test_package_install.py` checks the packaged copies.
- **Tests** live in a dedicated `tests/mode_switch.cpp` rather than growing `canvas_focus.cpp`, so the include-trick suites stay focused; the gate list is unchanged.

**M6 glasses PR checklist** (the full release pass is the end-to-end checklist in [`window-canvas.md`](window-canvas.md))
- [ ] Live switch monitors → canvas → monitors in direct stereo keeps SBS and the lease (no desktop flash, one renderer process); windows migrate and restore with tiling; the canvas chords are Omarchy's again; no hidden workspace on the canvas output; `layout.json`/`viewer.tsv` byte-identical.
- [ ] Notification cards in canvas mode sit inside the ring in the lower part of the view, over the windows, readable, and flick up/down dismiss and cycle; also after turning 180°.
- [ ] Mono HUD: cards show in the flat glasses view (`--display`) and the windowed preview in both modes.
- [ ] A new window pulses; one opened behind you shows the accent edge cue until you look at it (≤ 3 s).
- [ ] Overview mouse drag in the windowed preview: moves along the ring, snaps, is remembered, Ctrl+Z undoes, a still click selects.
- [ ] The stop-first fallback once (`kill -STOP` the renderer before switching): XR restarts in the new mode in the same presentation.
- [ ] The switch is refused with the laptop display off.
- [ ] The spectator shows the same picture as the glasses in both modes, across a switch.
- [ ] `make check-preview` pixel-identical (monitor mode unchanged).

### M7 — Interaction follow-up (after first glasses test) — **DONE** (2026-09-26; glasses checklist pending)

**Report** (Studio 0.4.0, Lua controls v6, direct stereo, laptop screen on): gaze selection and the halo work, but no window ever received real control focus (the pointer never entered a window, no clicking or typing), windows could not be positioned or resized from the glasses, and `SUPER+F` did nothing. `viewer.log`: `Canvas: N windows, staged none` throughout, one `staged 0x5848402512d0` at the end, `Canvas: no staged window to fill` × 4, `Gaze: dwell 500 ms … pointer follows (defaults)`, no `Canvas: land on`, `click` or `cursor crossed` line at all; `controls-settings.tsv`: `fit_all=CTRL+Up`, `fit_target=CTRL+Down`.

**Root causes** (read end to end: `steer()` → `navigateCanvas` → `Scene::land` / `explicitFocus` → `.hover` v4 → Lua `canvasHover`/`stageWindow` → `.windows` `stage` row → `chooseStaged`/`onStage`):
1. **The `fit_target` hotkey never focuses.** Lua publishes mode 2 for both the flick-in gesture and the `fit_target` key; the renderer maps mode 2 to `Verb::FlickIn`, and in canvas mode `Scene::flickIn` from Overview only *lands the camera* (`land()` sets no `focusRequest`), while from Work it toggles Fill, which needs a staged window ("no staged window to fill" × 4). Only the pose-socket verb `fit_target` (Studio's *Land on window* button) reaches `canvasFitTarget` → `focusHit` → `explicitFocus` → `.hover` v4. The same gap applies to `toggleOverview`'s landing (SUPER+TAB from Overview).
2. **Nothing is staged at start.** Lua stages only on a `.hover` v4 line with a new pointer serial (or for a dialog in `adopt`); the renderer's `chooseStaged` falls back to the `focus_history_id 0` row among *canvas members* only, and at start the focused window is Studio on the laptop, so `stagedName` stays empty. Without a `stage` row there is no region capture, no XR cursor, no cursor confinement, and Fill is refused. The one `staged 0x5848402512d0` was Lua's dialog rule (`dialogParent` → `stageWindow`, the Signal window shared a pid with a member); the closing `staged none` is Stop's restore (`onMove` → `release`), not a mid-session drop.
3. **Dwell is halo-only by design** (§5.6) and nothing told the user which action confirms; the M3 checklist item "click/type via `fit_target`" was never exercised with the hotkey.
4. **Transport is fine**: `publishHover` writes v4 in direct mode (`main.cpp` `tick()`: canvas mode is not gated on `direct`), Lua's `canvasHover` warps once per serial (`testHoverV4Stage`).
5. **No move/resize in the glasses.** M6's drag is SDL-only; in the glasses `SUPER+left-drag` is Omarchy's `hl.dsp.window.drag()` (`tiling.lua:70`, `{mouse=true}`), which moves the real window off the stage origin on the canvas output (the ring panel does not move, the region and confinement rectangles go stale). `SUPER+right-drag` (`hl.dsp.window.resize()`) resizes the real window, which the ring follows through the list and the buffer, but a top-left corner drag also moves it off the origin.
6. **`Could not start stereo … Connect or turn on a computer display`** comes from `place_spectator()` (`backend.py`): no enabled non-XR display at that moment. Not canvas-caused: the canvas output is excluded by its `OMXR-` prefix like every XR output. It follows a Stop that failed to leave side-by-side (`display-events.jsonl` `stereo-restore-skipped` right before), after which the laptop display was still off; the rollback then also lacks a display (`Workspace release: There is no computer display…`). The robustness gap is that an unplaceable spectator aborts the whole stereo start.

**Changes** (user decisions in bold):
- **Gaze focus = dwell + confirm.** Dwell keeps selecting and haloing only. A *confirm* lands on the gazed window (dwell within 2 s, else the search selection, else the staged, else MRU: `landingTarget`) = stage + raise + keyboard focus + warp the real pointer to the gaze point (the existing `canvasFitTarget` → `explicitFocus` → `.hover` v4 → Lua `selectCanvasWindow` path). Triggers: the `fit_target` hotkey (Ctrl+Down by default; in canvas mode Lua publishes the new canvas mode **18 `confirm`** for it instead of 2, so the flick-in gesture keeps mode 2), the pose verb `fit_target` (unchanged), and a **three-finger single tap** on the touchpad: the tap that the existing double-tap bind already sees is confirmed on the 33 ms timer 400 ms after it when no second tap followed (a double tap within 400 ms stays recenter + pointer release; a swipe cancels as today). Flick-in and SUPER+TAB landings from Overview also focus the landed window (`Scene::flickIn`/`toggleOverview` set `focusRequest`; the View uses the gaze pixel when the gazed window is the landed one, else the buffer centre). A small hint raster ("Ctrl+Down or a three-finger tap to focus", the key read from `controls-settings.tsv` line 3 next to `canvas.tsv`) is drawn under the dwelled window's label the first three dwells of a session and never after the first confirm.
- **Always a staged window.** Lua `ensureStaged()` (33 ms tick, at most every 0.5 s): when `canvas.staged` is nil or not a live member and members exist, `stageWindow(mru, mon, quiet=true)` on the member with the lowest `focus_history_id` — quiet = no `hl.dsp.focus` and no cursor move, so the keyboard and pointer stay where they are until a confirm. Covers canvas entry, the staged window closing (`onClose`) and leaving (`release`). Renderer: `adoptList` lands on the staged window when the scene has not landed yet or the landed window is gone.
- **SUPER+F with no staged window** (or one not on the stage) confirms first (`canvasFitTarget`) and fills when the next `.windows` list shows that window staged (`fillAfterStage`, expires after 2 s).
- **Move in the glasses: SUPER + left-drag on the staged window moves it along the ring.** Lua takes over `SUPER + mouse:272` in canvas mode (unbind + press bind + `{release=true}` bind, restored on exit as `hl.bind("SUPER + mouse:272", hl.dsp.window.drag(), {mouse=true, description="Move window"})`, the M4 takeover pattern) and publishes `.drag` (`v2 owner seq id dx dy active stamp`, the `.pan` codec): cumulative pointer travel since the press including the confinement overflow, written on the 33 ms tick while held, `active 0` on release. A release bind matches the modifiers held at release, so the press also installs a plain `mouse:272` `{release=true}` bind for the drag's lifetime (letting go of SUPER first), and 3 s without pointer travel end a drag whose release was lost (review fix; the renderer keeps trusting `.drag`). The renderer (`LiveControls::updateDrag`, the `.pan` reader generalised) drives `Scene::dragBegin(stagedName)`/`dragBy(dx·outputScale/zoom, dy·outputScale/zoom)`/`dragEnd()` — snap 20 px, row band, undo checkpoint, memory save as in M6 — and after the end chases the window like nudge when it left the view. The real window never moves on the canvas output, so confinement and the region stay valid.
- **Resize in the glasses.** `SUPER + right-drag` stays Omarchy's `hl.dsp.window.resize()` on the real window (the buffer drives `rect`, the region reopens, confinement follows the snapshot); Lua re-clamps the staged window 0.5 s after its size or position last changed (back to the stage origin, width ≤ band, height ≤ output) so a corner drag cannot leave it off the origin. **`SUPER+CTRL+arrows` resize the staged window in 100 px steps** (Left/Right width −/+, Up/Down height −/+, clamped to the band, ≥ 100 px): free chords in Omarchy (no restore needed; bound only while the canvas runs through the canvas key set), applied by Lua directly (`resizeStaged`), the renderer follows through the list and the Fill three-case rule ("resized in between fills again").
- **Stereo start**: `direct_arguments()` no longer aborts when the spectator cannot be placed; it logs `display_event("spectator-skipped", …)`, starts without `--spectator` and reports "Recording window skipped: no computer display" in the start message; `set_spectator(True)` (an explicit request) keeps raising. Documented in the troubleshooting section.
- F1 help rows (View: "Ctrl+Down / 3-finger tap — focus the gazed window"; Arrange: "SUPER+left-drag — move along the ring", "SUPER+right-drag — resize", "SUPER+CTRL+arrows — resize 100 px"), `docs/window-canvas.md` (keys table, "The pointer and other keys", troubleshooting, the M7 glasses checklist), README key line, Studio's *Land on window* help text.
- `live_controls.hpp`: mode ≤ 18 in canvas mode; `.drag` parser; `LiveControls` unlinks `.drag` on exit.

**Acceptance**: with three windows on the canvas in direct stereo, `viewer.log` shows `staged 0x…` from the first list; Ctrl+Down (and a three-finger tap) on a gazed window logs `Canvas: land on`, the window is staged, raised and focused, the pointer lands at the gaze point and typing arrives, within 50 ms of the Lua tick; SUPER+F with no staged window fills the gazed window; SUPER+left-drag moves the staged window along the ring in 20 px steps, Ctrl+Z undoes and the place survives a restart; SUPER+right-drag and SUPER+CTRL+arrows resize it and the panel follows; the staged window closing stages the next MRU; the hint shows on the first dwells only; a stereo start with the laptop display off proceeds without the recording window; every gate green, `check-preview` pixel-identical, no new luacheck/function-length warnings.

**Tests**: `tests/controls.lua` — `testConfirmHotkey` (Ctrl+Down publishes 18 in canvas mode, 2 in monitor mode; the flick still publishes 2), `testTapConfirm` (single tap → 18 after 0.4 s; double tap → 3 and no 18; a swipe cancels), `testEnsureStaged` (entry with members and `staged=nil` → the lowest `focus_history_id` member is staged quietly: move/resize/move/bring_to_top, no focus, no cursor.move; `onClose` of the staged → the next MRU; no members → nothing), `testStageDrag` (SUPER+mouse:272 taken over and restored with `hl.dsp.window.drag()` `{mouse=true}`; press → `.drag` id+1 active 1; cursor motion incl. overflow → cumulative dx/dy; release → active 0; the real window never moves), `testResizeKeys` (SUPER+CTRL+arrows ±100 clamped to the band; not bound outside canvas mode), `testStageReclamp` (a moved/oversized staged window is put back 0.5 s after it settles, not while still changing). `tests/canvas_focus.cpp` — `stagedNoneAtStart` (a mailbox list without a `stage` row and with `focus_history_id 0` on a non-member reproduces `staged none`; mode 18 on a gazed window publishes v4 with `restageRequested`; the next list with the `stage` row clears it and lands), `flickInFocuses` (Overview flick-in / SUPER+TAB landing sets `hoverOutput` and a new pointer serial), `fillLandsFirst` (Fill with no stage → v4, then `.fill` after the stage row), `stagedCloseLandsNext`, `stageDrag` (`.drag` lines move the staged window in Work with snap, checkpoint and memory; a drop outside the rows reverts), `confirmHint` (label texture present for the first three dwells, gone after a confirm). `tests/test_studio.py` — `test_stereo_start_skips_unplaceable_spectator`. `tests/test_canvas.py` unchanged behaviour asserted for the canvas output exclusion in `place_spectator`.

**Shippable**: yes (0.4.1 candidate after the glasses checklist).
...
- [ ] Stop: SUPER+mouse:272 is Omarchy's *Move window* again (`hyprctl binds -j`), SUPER+CTRL+arrows unbound.
- [ ] Stereo start with the laptop display off: starts without the recording window; the message says so.
- [ ] A `{release=true}` bind on `SUPER + mouse:272` fires on real Hyprland (the drag ends with `active 0`), and `hl.unbind` removes the mouse bind; releasing SUPER before the button ends the drag through the plain release bind; after Stop SUPER+CTRL+Left/Right are Omarchy's group focus keys again.
- [ ] The confirm hint is readable under the label and its accent tint visible; Studio's Input tab title "Fit selected monitor / focus window" fits its 180 px column.

#### M7 notes

Decisions taken while implementing M7:
- **Controls version stays 6.** An older adapter degrades gracefully: the hotkey still publishes mode 2 (lands, and from Overview now focuses through `landFocused`), there is no auto-staging and no ring drag. No version gate churn; the renderer accepts modes ≤ 18 only in canvas mode (≤ 7 in monitor mode).
- **Mode 18 `confirm`** is published by Lua for the `fit_target` hotkey (and `omarchy_xr_controls.fit_target`) in canvas mode and by the three-finger single tap (`settleTap` on the 33 ms tick, 400 ms after a tap with no second one; `enterCanvas` clears `lastTap`). The renderer maps it to `Verb::FitTarget`; the target is the gaze, then the selection, then `landingTarget` (staged, else MRU) — `fitTargetName`, also logged as `Canvas: confirm 0x…`. The flick-in gesture keeps mode 2. Landings from Overview (flick in, SUPER+TAB) go through the new `Scene::landFocused`; `land()` itself still never sets `focusRequest`, and the pointer goes to the gaze pixel only when the gazed window is the one focused, else the buffer centre.
- **`.drag` codec**: `v2 <session> <seq> <id> <dx> <dy> <active> <stamp>`, the `.pan` codec (`%.9f`, cumulative travel since the press including the confinement overflow, the ±1000 delta clamp). `LiveControls` reads it only in canvas mode through a shared `updateCumulative` (the `.pan` reader generalised), resets it on a mode change and unlinks it at construction (stale file) and exit. The renderer scales by `outputScale / max(zoom, .05)`, acts only in Work/Fill with the staged window on the stage, holds `interactionUntil` 0.4 s while dragging, ends a held drag before a new one, and chases the window after the drop when it left the view (`Scene::chase`, shared with nudge).
- **Drag takeover is global**: Lua cannot fall through to Omarchy's `window.drag()` from a callback, so while the canvas runs SUPER+left-drag does nothing on the laptop. Restore rebinds `hl.bind("SUPER + mouse:272", hl.dsp.window.drag(), {mouse=true, description="Move window"})` on leave and in the load-time `retireCanvas`.
- **SUPER+CTRL+Left/Right are not free** (the plan assumed they were): Omarchy's `tiling.lua:84-85` binds them to *Move grouped window focus left/right*. They are canvas keys with `restore` data like the takeovers; Up/Down are free. All four are always bound while the canvas runs (not governed by the takeover switch).
- **Quiet staging**: `ensureStaged` (at most every 0.5 s after `syncTiers`) stages the member with the lowest `focus_history_id` via `stageWindow(address, mon, quiet=true)` — no `hl.dsp.focus`, no cursor move. The renderer's `View::stagedLanding` lands on a newly staged window only when nothing has landed yet, and on the next staged one when the landed window is gone *and* the scene is in Work/Fill (a close in Overview does not pull the camera into Work); it never sets `focusRequest`.
- **Re-clamp settle 0.5 s**: once the staged window's geometry has been unchanged for 0.5 s (never during a drag) Lua resizes it to the band and moves it to the stage origin. A clamp the window refuses (minimum size) is tried once per geometry, not every 0.5 s.
- **SUPER+F lands first**: `fillAfterStage` is set when Fill finds nothing on the stage and a confirm target exists; the fill runs when the stage row appears with no restage pending, within 2 s. Without any target it falls through to `fillToggle` ("no staged window to fill").
- **Hint**: `canvas::readConfirmKey` reads line 3 of `controls-settings.tsv` next to `canvas.tsv` with Lua's `settingKeys` validation (default "CTRL + Down", empty = no key → "Three-finger tap to focus"). It shows 2.5 s per distinct dwell target, skips the staged window, stops after 3 hints or the first confirm (`Scene::noteConfirm` in `explicitFocus`, so a pointer crossing ends the hints too) and sits 8 px inside the window's top edge under the label (below the label would cover the window's top edge anyway). The static F1 help hard-codes "Ctrl+Down", while the hint uses the configured chord.
- **Spectator degrade**: `direct_arguments()` catches `place_spectator()`'s `RuntimeError`, logs `display_event("spectator-skipped", …)`, stores the reason in `spectator_skipped` (`status()["spectatorSkipped"]`, the Recording card hint in the urgent colour) and starts without `--spectator`; the start message adds "Recording window skipped: no computer display." `set_spectator(True)` still raises. `place_spectator` excludes every `OMXR-` output, so canvas mode cannot cause the failure.
- **Lua locals**: the main chunk of `xr-controls.lua` is ~7 locals below Lua's 200-local limit; M7 helpers live in `do` blocks and a `drag` table. Later Lua work must do the same.

Measured (2026-09-26, live smoke on a headless `OMXRTEST-m7` output 2560×1440@60, three 1280×800 30 fps spike clients parked, the windowed renderer with `--canvas-windows-file`, laptop only):
- A list without a `stage` row (the clients' `focus_history_id` 6–8, the user's window 0): `Canvas: 3 windows, staged none`, Overview ladder 3 × 10 Hz — the reported "staged none" reproduced renderer-side (in a real session Lua's `ensureStaged` now writes the `stage` row).
- The list rewritten 6 s later with the first client staged at the origin: `Canvas: 3 windows, staged 0x…` on the next poll, the camera landed on it (`Canvas: ladder near 40 Hz`), the staged window focused 60 Hz with its region source live (`region dmabuf 46.2 fps, request to ready 33.6 ms`; 475 of 542 frames from the region), the two others near 40 Hz (~36.9 fps), 143/300 Mpix/s, VRAM ~74 MB.
- `make smoke-canvas` afterwards: renderer exit 0, region frames yes.
- Clean-up: clients killed, `OMXRTEST-m7` removed, the previously active window refocused.

### M8 — Full cylinder (after second glasses feedback) — **DONE** (2026-09-26; glasses checklist pending)

**Report** (user, wearing the glasses, after M7): "I want more than a ring, I want a cylinder and be able to position windows also on y, not only x." Today the canvas is a 2D plane on the cylinder confined to three rows around eye height: `canvas_placement.hpp` `inBand` / `rows {0,1,-1}` / three shelves, `canvas_model.hpp` `rowFor` clamped to −1..1, `Scene::pan` clamped to ±1.5 rows, nudge/drag/arrange "stay within the three rows", the radar's three lanes.

**User decisions** (bold) and design:
- **Height unbounded, with vertical scroll.** Windows sit at any y. The head looks up and down within the view as before; beyond that the cylinder scrolls: `Camera::scrollY` (§4.3), a plain translation on the cylinder surface in projected px (windows stay upright and undistorted, eye-level text stays 1:1). Drivers: **`SUPER+wheel`** (takeover of Omarchy's *Scroll active workspace forward/backward* = `hl.dsp.focus({workspace="e±1"})`, in `TAKEOVER_KEYS` behind the takeover switch, restored on exit like the others), **`SUPER+CTRL+Page_Up/Page_Down`** (free chords **[verified: not in `hyprctl binds`]**, in `CANVAS_KEYS`, always bound in canvas mode), the **4-finger pan's vertical component** (the existing gesture and `.pan` codec; horizontal keeps turning the head along the ring; the 3/5-finger flicks and the three-finger taps are untouched), and `PageUp/PageDown` plus the middle-drag pan in the SDL window. New mode **19 `scroll`** with a direction token (§5.5); the renderer's `Verb::Scroll` calls `Scene::scrollBy(fraction·viewH)`: a wheel notch 0.2 view heights, a page 0.8. The scroll is clamped to the live windows' vertical extent plus half a view (`scrollLimits`), so the user can put the top row at the bottom of the view and open windows into the space above, but cannot drift into the void.
- **Every landing scrolls its window to eye level**: `Scene::focusOn(x, y)` (`targetFocusX/Y` and `targetScrollY = y`) replaces the `targetFocusX/Y` pairs in `land`, `overview`, `recenter`, `follow`, `chase`, `followSelection`, `restoreCamera`, `searchClose(revert)`; aims then use projected y 0 (or `above` under the palette). `visibleShare` becomes `min(horizontal, vertical)` so focus follow and the post-move chase also react to a window above or below the view. `SUPER+arrows` (`neighbour`, the 45° rule already scores dy) therefore works across the full height.
- **Overview fits both axes** (already `min(viewW/spanX, 0.9·viewH/spanY)`, floor 0.08) and scrolls the bounds centre to eye level; the radar gains the vertical axis: each mark carries its projected top/bottom in view heights, the view is a rectangle (`viewDeg` × 1 view height), the strip scales to the occupied extent (`radarHeight` 72 → 96 px).
- **Placement near where I look**: `placeNew` starts at `Scene::lookPoint()` (the canvas point under the view-centre ray: `heading` and a new `pitch`, both set by the View from `currentView()`; unprojected through zoom and scroll) and spirals outward on the periodic-x / unbounded-y plane (§5.1: cells of step `(w+gap, h+gap)`, rings 1..16, sorted by distance, deterministic ties), no overlap, snapped to the 20 px grid, no `inBand`. `Memory::claim` keeps y as stored; the dialog rule (start at the parent centre) is unchanged; the "walk each row's centre line" fallback goes.
- **Arrange in 2D blocks**: grouping by category, then class, MRU inside is unchanged; `arrange(windows, headingX, gazeY, …)` packs rows of `rowHeight` around the grid row nearest the look point, in the order 0, +1, −1, +2, −2 … (unbounded), each row an arc `arcW` (§5.1) centred on the heading; leftovers fall back to `placeNew`. From Work or Fill it still ends in Overview over the block.
- **Free vertical moves**: `nudgeBy`, `dragEnd` (SDL Overview drag and the SUPER+left-drag), `summon` drop the `inBand` check; snapping, the undo checkpoint and the memory note stay.
- **Culling**: `adaptive::project` already clips against the vertical frustum planes, so windows above or below the view are `visible=false` and idle in the ladder (budget ladder unchanged); `Scene::cull` adds a vertical candidate bound (one view height beyond the pitched view centre, `visibleIndices` overload) so a tall layout does not draw, label or gaze-test every window at the heading; `offView` gains the same vertical test for the new-window edge cue.
- Docs: F1 help rows (View: "SUPER+wheel / SUPER+CTRL+PgUp/PgDn — scroll", "4-finger pan — pan / scroll"; Arrange: "SUPER+left-drag — move (also up and down)"; `helpHeight` 560 → 640), `docs/window-canvas.md` (cylinder wording, keys table, moving windows, radar, windowed keys, troubleshooting, the M8 glasses checklist), README key line, Studio's takeover help text and `CANVAS_CHORDS` (`SUPER + CTRL + Page_Up/Page_Down`). Controls version stays 6 (an older adapter has no scroll keys; everything else works).
- Monitor mode: untouched (`check-preview` pixel-identical); `canvas-memory.tsv` files from before M8 load unchanged (y was always stored; nothing is clamped on load; a four-field camera row gets `scroll = y`).

**Acceptance**: in direct stereo with six windows (two of them nudged above and below the old rows), `SUPER+wheel`, `SUPER+CTRL+Page_Up/Down` and a vertical 4-finger pan scroll the cylinder smoothly, upright, with sharp text at eye level, and stop half a view beyond the topmost and lowest window; `SUPER+Up/Down` reach the windows above and below and each landing (confirm, search Enter, ALT+TAB, neighbour) brings its window to eye level; a new window opens next to the gaze point in x and y without overlap; Ctrl+A lays the windows out in a compact block around the gaze and the Overview shows all of it, the radar its height and the view rectangle; SUPER+left-drag and SUPER+SHIFT+Up/Down move a window freely in y with snap and Ctrl+Z; a dialog still opens over its parent; windows out of view above or below are idle in the ladder (`Capture:` lines, `Canvas: ladder`); stop restores `SUPER+mouse_down/up` to Omarchy's workspace scroll (`hyprctl binds -j`); a pre-M8 `canvas-memory.tsv` restores every window where it was; every gate green, `check-preview` pixel-identical, no new luacheck or function-length findings.

**Tests**: `tests/canvas_model.cpp` — `scrolling` (projection with `scrollY`, `focusOn` puts the point at eye level, the anchor stays put under `zoomAt` with a scroll, `moving()` covers the scroll, `visibleIndices`/`offView` vertical overloads, `fitBounds` of a tall layout fits 0.9 of the view height). `tests/canvas_placement.cpp` — `placement` (spiral: 500 random sizes at random y stay apart and deterministic, the first free cell is the nearest by distance, ties prefer eye height then right; a start far above eye height places there), `arranging` (200 windows form one block: no overlap, rows on the grid around the look row, the arc within `arcW`, deterministic, < 5 ms), `summoning` (free y). `tests/canvas_memory.cpp` — five-field camera row round trip, four-field row loads with `scroll = y`. `tests/canvas_focus.cpp` — `verticalScroll` (mode 19 tokens through `.controls` in canvas mode only; `Verb::Scroll` and a vertical `.pan` move `targetScrollY`, clamped at the extent; the landed window's projected centre is at eye level after land/neighbour up/search/switcher; a window three grid rows up is reached by `SUPER+Up` and scrolled to eye level; the camera row saves and restores the scroll; the stage drag and the Overview drag keep a drop above the old band; `follow` of a window below the view scrolls; windows above the view are `visible=false` and idle). `tests/controls.lua` — `testScrollKeys` (canvas mode: `SUPER + mouse_up/down` publish 19 with `up`/`down`, `SUPER + CTRL + Page_Up/Down` with `pageup`/`pagedown`; monitor mode: the Page keys are unbound and the wheel chords carry Omarchy's `focus` workspace `e-1`/`e+1` with their descriptions; the takeover flag off leaves the wheel with Omarchy and the Page keys bound; takeover count 15). `tests/canvas_preview.cpp` — 24 help rows fit `helpHeight`; the radar still lights. `tests/test_input_settings.py` — the Page chords are reserved.

**Shippable**: yes (0.4.1 candidate together with M7 after the glasses checklists).
...
Decisions taken while implementing M8 (deviations from the entry above are marked):
- **Scroll is its own camera component** (`Camera::scrollY/targetScrollY`, projected px, `y' = focusY + (y−focusY)·zoom − scrollY`), eased with the focus and counted in `moving()`; `unprojectY` is the inverse. `Fit` carries `scrollY` (= the fitted `focusY`, added after `wide` so older brace initialisers compile). `targetView` includes `targetScrollY`, so follow, chase and the eye-level checks judge the landed view.
- **Landings go through `focusOn(x, y)`** (land, overview, recenter, follow, chase, restoreCamera, `searchClose(revert)`); `followSelection` aims at `above` under the palette. The scroll is carried through `headingPoint`, the Return snapshot, `searchAttach`, `latch`, `noteCamera` and `snap`. `noteCamera` saves `targetScrollY` and treats a scroll-only change as a new camera (else it was never written).
- **Placement**: `spiralCells` (distance, then smaller |j|, then right, then down) drives `placeNew` from `Scene::lookPoint()`; summon keeps `headingPoint()`. `arrange(windows, headingX, gazeY, …)` packs rows in the order base, +1, −1, +2, … (at most 64 rows) and falls back to `placeNew(headingX, gazeY)`. Deviations: rows fill from the gaze row outward, so the most recent group starts at the left edge of the gaze row (not the block's top-left); a row ends at `startX + arcW + gap` (the arc already holds one trailing gap per window, else a window as wide as the arc would fit no row); `shelve` takes no `startX` (the shelf cursor starts there). `placeNew` and `summonRect` no longer take `Metrics` (only `inBand` used it).
- **View ray**: `Scene::viewRay()` intersects the view-centre ray (heading, pitch) with the cylinder from where the eye is: the ring centre in Overview/Search, `depth` in front of the aim in Work/Fill, at the aim's height `aimY`. `lookPoint()` (placement, Arrange, the radar's centre) unprojects that point with `unprojectX/unprojectY`; `latch` and `headingPoint` use the same inverses.
- **Culling**: `visibleIndices`/`offView` take `centreY` and `halfHeightPx` (defaults behave as before); `Scene::cull(heading, halfSpan, pitchDeg)` bounds candidates to the rows the pitched view spans at the ray's reach (`viewBand()`, `tan(pitch ± fov/2)`), plus half a view height at that reach; the new-window cue tests against the same band without the margin. Deviation: `visibleShare`'s vertical window is centred on the view ray's y rather than eye level, so follow and chase judge where the head actually looks (identical at pitch 0 in Work). `tests/scene_seam.cpp` now counts the windows inside the band (80 small windows so the 24-occluder cap is still reached).
- **Inputs**: mode 19 with `up|down|pageup|pagedown` (`Scene::scrollFraction`: ±0.2 / ±0.8 view heights). Deviation: `.controls` is last-writer, so a wheel notch is published as `up|down:<run>:<count>` (a run starts with a turn or after any other mode press, its id the Lua serial); the renderer scrolls every notch of the run since the line it last read (at most 5 at once), so a fast spin within one 33 ms poll is not lost; plain `up|down` stay one notch; `SUPER + mouse_up/mouse_down` in `TAKEOVER_KEYS` (restored as Omarchy's `e-1`/`e+1` *Scroll active workspace*), `SUPER + CTRL + Page_Up/Page_Down` in `CANVAS_KEYS` without restore; SDL `PageUp/PageDown` ±0.8. Controls version stays 6. `tests/controls.lua` reaches its run list through a forward-declared `local testScrollKeys` in a `do` block (one test-file local; the config gains none).
- **Radar**: `Mark{offsetDeg, widthDeg, top, bottom, staged}` with `top/bottom` the projected edges in view heights (scroll included), keyed to 0.05; the strip is 96 px, scaled to the occupied extent (0.5..4 view heights either side), the view a rectangle of ±0.5 view heights. `rowFor` stays (unclamped) as the arrange grid function only.
- **Help card**: 24 rows. Deviation: the scroll row's keys ("SUPER+wheel / SUPER+CTRL+PgUp/PgDn") did not fit the 200 px key column, so the card grew to 1120 × 640 px (columns 544 px apart, keys 340 px, actions 180 px) instead of only 560 → 640 in height.
- **Review tests**: `canvas_focus` `pitchedLook` (`View::pitchDeg` sign, the look point above/below eye level from the ring centre and from Work's depth, a new window and the Arrange block at that height) and `radarHeights` (mark heights with the scroll, `radarKey` on a vertical move); `stageDrag` drops above the old rows, the nudge test goes up past them; `scrollVerbs` covers wheel runs; `canvas_model` `scrolling` covers `zoomAt` under a scroll and the tall `fitBounds` zoom.
- `canvas-memory.tsv`: the renderer re-wraps window x into `[0, P)` when it saves (a pre-M8 row at x −640 comes back as 12 931.68, the same place on the ring); y is kept as stored.

Measured (2026-09-26, live smoke on a headless `OMXRTEST-m8` output 2560×1440@60, three spike clients (one 1280×720 staged, two 960×600 at 30 fps parked), the windowed renderer with `--canvas-windows-file`, laptop only):
- A seeded pre-M8 `canvas-memory.tsv` (four-field camera row, windows at y −360, −3300 and +2700) loaded unchanged: after the run the window rows kept y −360/−3300/2700 (x re-wrapped by one period, see above) and the camera row was written with five fields (`camera 0 0 1 -3377.84`, the upper `scrollLimits` bound after the eight PageUp presses).
- At start `Canvas: ladder focused only`: the staged window focused 60 Hz (59.3 fps, region dmabuf, request to ready 33.4 ms), the windows above and below idle (0 frames).
- Four SDL `PageDown` presses (wtype, renderer focused for about 1 s, focus restored by address): the lower window went near 40 Hz (`Canvas: ladder near 40 Hz`), the upper stayed idle. Eight `PageUp` presses: the lower window went idle (a 256×161 thumbnail), the upper came in (far, then `Canvas: ladder near 40 Hz`).
- `make smoke-canvas` afterwards: renderer exit 0, region frames yes.
- Clean-up: clients killed, `OMXRTEST-m8` removed, the previously active window refocused.
...
- **D6** *Resolved (M6)*: the notification HUD is constructed whenever a pose socket exists and is drawn in mono, the spectator and the flat glasses view; `make smoke` (no pose socket) and the `check-preview` stills (their own `Hud`) are unaffected.
- **D7** *Resolved*: no Hyprland fullscreen; Fill only (S10 shows why the rebind, not just the rule, is needed).
- **D8** Ring defaults: R 2.4 (≈ native pixel density), row height 0.8·vFOV (850 px), ~~max 3 rows~~ **amended M8 (user, wearing the glasses): the canvas is a full cylinder, height unbounded with vertical scroll; the row height is the arrange grid only**, Overview fits the view in both axes (else ±45°).
- **D9** *Resolved*: non-canvas windows appear in search with "bring to canvas".
- **D10** *Resolved (M6)*: live single-process switch (`mode:` datagram, acknowledged by the `.stats` `mode`), with stop-first (stop XR, restart in the same presentation) as the fallback when the renderer does not acknowledge within 3 s.
